...
export async function runPipeline({ skipMultiTurn = false, preFilledBrief = null, sessionId = null } = {}) {
  ...
  try {
    ...
    session = (await getSession(session.sessionId)) || session; // Ensure freshest state
    await addLog(session.sessionId, 'IntakeAgent', 'Brief collected and validated.');
    await updateSession(session.sessionId, { pipelineStatus: 'research', currentStep: 1 });

    // ── Phase 2: Research ────────────────────────────────────────────────────
    printPhaseHeader(2, 'Research Agent — Building Fact Sheet');
    await addLog(session.sessionId, 'ResearchAgent', 'Starting deep research on topic...');
    session = await runResearchAgent(session);
    session = (await getSession(session.sessionId)) || session;
    await addLog(session.sessionId, 'ResearchAgent', 'Fact sheet synthesized and saved.');
    await updateSession(session.sessionId, { pipelineStatus: 'writing', currentStep: 2 });
    ...
    while (reviewStatus !== 'approved' && reviewCycles < MAX_REVIEW_CYCLES) {
      ...
      await addLog(session.sessionId, 'WriterAgent', `Generating content drafts (cycle ${reviewCycles})...`);
      const feedback = session.reviewNotes?.at(-1) ?? null;
      session = await runWriterAgent(session, feedback);
      session = (await getSession(session.sessionId)) || session;
      await addLog(session.sessionId, 'WriterAgent', 'Drafts ready for human review.');

      printPhaseHeader(4, `Review Agent — Human-in-the-Loop Approval (cycle ${reviewCycles})`);
      await updateSession(session.sessionId, { pipelineStatus: 'review', currentStep: 3 });
      await addLog(session.sessionId, 'ReviewAgent', 'Waiting for human approval...');
      session = await runReviewAgent(session);
      session = (await getSession(session.sessionId)) || session;
      await addLog(session.sessionId, 'ReviewAgent', `Draft ${session.reviewStatus}.`);
    ...
    if (reviewStatus !== 'approved') {
      ...
      await updateSession(session.sessionId, { reviewStatus: 'escalated', pipelineStatus: 'escalated' });
    }

    // ── Phase 5: Eval ────────────────────────────────────────────────────────
    if (session.pipelineStatus !== 'escalated' && session.reviewStatus === 'approved') {
      printPhaseHeader(5, 'Eval Runner — Scoring & Analysis');
      console.log(chalk.blue('  Starting Eval Phase...'));
      await updateSession(session.sessionId, { pipelineStatus: 'eval', currentStep: 4 });
      await addLog(session.sessionId, 'EvalRunner', 'Running automated quality scoring...');
      session = await runEvalRunner(session);
      session = (await getSession(session.sessionId)) || session;
      await addLog(session.sessionId, 'EvalRunner', 'Evaluation complete.');
    }

    // ── Phase 6: Publisher ───────────────────────────────────────────────────
    if (session.pipelineStatus !== 'escalated' && session.reviewStatus === 'approved') {
      printPhaseHeader(6, 'Publisher Agent — Formatting & Saving');
      console.log(chalk.blue('  Starting Publisher Phase...'));
      await updateSession(session.sessionId, { pipelineStatus: 'publish', currentStep: 5 });
      await addLog(session.sessionId, 'PublisherAgent', 'Formatting and saving final output...');
      session = await runPublisherAgent(session);
      session = (await getSession(session.sessionId)) || session;
      await addLog(session.sessionId, 'PublisherAgent', 'Output published successfully. Pipeline finished.');
    }
    ...
    if (session?.sessionId) {
      await logError(session.sessionId, 'orchestrator', err);
